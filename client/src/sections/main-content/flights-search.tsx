...
export const FlightsSearch: React.FC<FlightsSearchProps> = ({ controller, setController }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, helpers) => {
            try {
                ...
                const result: QueryObserverResult<any, Error> = await refetch();

                if (result.isSuccess && result.data && result.data.flights?.length > 0) {
                    toast.success('Flights listed successfully');
                } else {
                    toast.error('No flights found');
                }
    ...
    return (
        <Stack sx={{ width: '100%', bgcolor: 'white', borderRadius: '10px', px: 2, py: 3 }}>
            <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 1.4 }}>
                <IoAirplane size={20} />
                <Typography variant="body2" fontWeight={600} color="text.secondary">BOOK YOUR FLIGHT</Typography>
                <Box sx={{ flexGrow: 1 }} />
                <Box sx={{ '& button': { borderRadius: '0', px: 2.6, py: 1, fontWeight: 600, fontSize: 11 } }}>
                    {['Round trip', 'One way'].map((item, index) => (
                        <Button
                            key={index}
                            variant="contained"
            ...
            <Grid container sx={{ width: '100%', mt: 3.4 }}>
                <Grid item xs={3}>
                    <InputSearchField
                        error={Boolean(formik.touched.from && formik.errors.from)}
                        fullWidth
                ...
                <Grid item xs={3}>
                    <InputSearchField
                        ...
                        name="to"
                        onBlur={formik.handleBlur}
                        onInputChange={(e: any) => formik.setFieldValue('to', e.target.id)}
                        value={formik.values.to}
                        disabled={!controller.filterMode}
                        placeholder="To"
                        startIcon={<BiSolidPlaneLand />}
                ...
                <Grid item xs={3}>
                    <DateField
                        ...
                        onBlur={formik.handleBlur}
                        error={Boolean(formik.touched.departureDate && formik.errors.departureDate)}
                        helperText={formik.touched.departureDate && typeof formik.errors.departureDate === 'string' ? formik.errors.departureDate : undefined}
                        name="departureDate"
                    />
                </Grid>
                <Grid item xs={3}>
                    {controller.journeyType === 'Round trip' ? (
                        <DateField
                            ...
                            onBlur={formik.handleBlur}
                            error={Boolean(formik.touched.returnDate && formik.errors.returnDate)}
                            helperText={formik.touched.returnDate && typeof formik.errors.returnDate === 'string' ? formik.errors.returnDate : undefined}
                            name="returnDate"
                        />
                    ) : (
                        <Box
                        ...
                        >
                            <FaPlus size={17} color="#6B7280" />
                            <Typography variant="body2" fontSize={{ xs: 11, sm: 14 }}>Add Return</Typography>
                        </Box>
                    )}
                </Grid>
